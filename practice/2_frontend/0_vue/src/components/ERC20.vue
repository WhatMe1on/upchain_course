...
// import bankAddr from '../../deployments2/dev/Bank.json'
//TODO 这边abi输出和address要做成地址传到这个项目里面
...
export default {
  ...
  methods: {
    ...
    async initContract() {
      this.erc20Token = new ethers.Contract(TokenAddress, 
        erc2612Abi.abi, this.signer);

      this.bank = new ethers.Contract(BankAddress, 
    ...
    readContract() {
      ...
      this.bank_account = BankAddress; 

      this.erc20Token.balanceOf(this.bank_account).then((r) => {
...
<template>
  <div >
    <br /> bank Token 余额 : {{ bank_balance }} <br>
